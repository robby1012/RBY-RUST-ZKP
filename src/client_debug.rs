...
async fn main() {
    ...
    println!("[debug] Register success, verify test, enter your password");
    stdin()
        .read_line(&mut buf)
        .expect("Could not get the username from stdin");
    let password = BigUint::from_bytes_be(buf.trim().as_bytes());
    ...
    println!("=============================LOGIN==================================="); 
    println!("\n[debug] Random secret (k) generated\n");
    println!("[debug] k (Secret Number): {:?}\n",&k); 
    println!("[debug] k (Secret Number) Byte: {:?}\n",&k.to_bytes_be()); 
    println!("[debug] compute r1 & r2 based on secret number\n"); 
    println!("[debug] r1 (byte): {:?}\n",&r1.to_bytes_be());  
    println!("[debug] r2 (byte): {:?}\n",&y2.to_bytes_be());
    println!("[debug] Sending compute challenge to server");
    ...
    let response = client
        ...
        .into_inner();
    println!("[debug] Server sent challenge request");
    ...
    println!("[debug] solving challenge request\n");
    println!("[debug] s: {:?}\n",&s);
